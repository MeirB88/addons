...
logging.warning("Starting login sequence1")
...
if not os.path.exists(db_path):
     os.makedirs(db_path)
logging.warning("Starting login sequence2")
def logout():
...
logging.warning("Starting login sequence")
...
while True:
    ...
    try:
        ...
        logging.warning("Err in recive:"+str(e))
    ...
    if event:
        ...
        if event.get('@type') == 'updateAuthorizationState':
            ...
            if authtype == "authorizationStateWaitTdlibParameters":
                logging.warning('send: setTdlibParameters')
                timeout=0
                ...
                logging.warning('send: checkDatabaseEncryptionKey')
