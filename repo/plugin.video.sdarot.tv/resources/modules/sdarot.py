...
user_dataDir_pre = xbmc.translatePath(Addon.getAddonInfo("profile"))
user_dataDir_img=os.path.join(user_dataDir_pre,'images')
if not os.path.exists(user_dataDir_img):
    os.makedirs(user_dataDir_img)

t_path = xbmc.translatePath(xbmcaddon.Addon().getAddonInfo('path'))
try:
    import dns.resolver
except:
    import shutil
...
def cache_ip(address):
    try:
        ...
        logging.warning('ERROR IN cache_ip:'+str(lineno))
...
class resolve_dns():
    ...
    def download_image(self):
        ...
        handlers += [urllib.request.HTTPHandler(), urllib.request.HTTPSHandler(), urllib.request.HTTPCookieProcessor(cookjar)]
        if Addon.getSetting('dns_solver')=='1':
            handlers = [urllib.request.HTTPHandler(), urllib.request.HTTPSHandler(context=ctx), urllib.request.HTTPCookieProcessor(cookjar)]
        ...
        request = urllib.request.Request(self.url.replace(' ','%20'),  headers=self.headers,data=url_encode(self.data).encode("utf-8"))
        logging.warning('self.url:'+str(self.url))
    ...
    def post(self):
        ...
        request = urllib.request.Request(self.url,  headers=self.headers,data=url_encode(self.data).encode("utf-8"))
    ...
    def image(self):
        ...
        request = urllib.request.Request(self.url,  headers=self.headers,data=url_encode(self.data))
...
def get_final_video_and_cookie(sid, season, episode, choose_quality=False, download=False):
    ...
    logging.warning('Donor test:'+token)
    if token == 'donor':
...
def get_links(tv_movie,original_title,season_n,episode_n,season,episode,show_original_year,id):
            ...
            req,cookie_new=resolve_dns(API + '/series/search/{0}/page/{1}/perPage/100'.format(name.replace('%','%25').encode('utf-8'), '0')).get()
