...
the python script "\plugin.video.thorrent\default.py" has left several classes in memory that we couldn't clean up. The classes include: class XBMCAddon::xbmcaddon::Addon

# https://stackoverflow.com/questions/26588266/xbmc-addon-memory-leak

"""
ADDONVS = xbmcaddon.Addon('plugin.video.thorrent')    # singleton
...
class addon():
    ...
    def VSlang(self, lang):
        ...
        #Bug avec accent xbmc.translatePath(xbmcaddon.Addon('plugin.video.thorrent').getLocalizedString(lang)).decode('utf-8')
