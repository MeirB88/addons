...
if Addon.getSetting("regex_mode")=='1':
    ...
    import re
from resources.modules.general import clean_name,check_link,server_data,replaceHTMLCodes,domain_s,similar,cloudflare_request,all_colors
type=['tv','subs']
...
def MyResolver(host):
  ...
  logging.warning('dns_value:'+dns_value)
...
class resolve_dns():
    ...
    def get_ip(self):
        ...
        logging.warning(data['Answer'][0]['data'])
        return data['Answer'][0]['data']
    def get(self):
        import requests
...
def get_user_cookie(cookie_pre=''):
    ...
    if username and password:
        ...
        if 'errors' in res:
            return cookie_pre
        #    xbmcgui.Dialog().ok('שגיאת התחברות', ', '.join(res['errors']).encode('utf-8'))
        if res['success']:
...
def get_links(tv_movie,original_title,season_n,episode_n,season,episode,show_original_year,id):
            ...
            try:
                ...
                name=requests.get(url2,timeout=10).json()[n_value]
            except:
                name=original_title
            ...
            if token == 'donor':
                ...
                res='480'
            if sd_link!='':
                all_links.append((original_title+'.S%sE%s'%(season_n,episode_n),'Direct_link$$$TELE%%%'+sd_link,'0',res))
