...
def _torrent_options(tv_movie,url_n,port):
        ...
        logging.warning('DEGUG:'+debug)
        binary_path=(os.path.join(xbmc.translatePath(Addon.getAddonInfo('profile')), 'resources', 'bin',"%s_%s" %(Platform.system, Platform.arch))).encode('utf-8')
        binary = "torrent2http"
        ...
        if Platform.system == 'windows':
            ...
            binary_path = ensure_android_binary_location(binary_path, os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(xbmc.translatePath('special://xbmc')))), "files", __addon__.getAddonInfo('id'), binary).encode('utf-8'))
        ...
        _path = xbmc.translatePath("special://profile/addon_data/%s/cache" % __addon__.getAddonInfo('id'))
        ...
        _path = xbmc.translatePath(__addon__.getSetting("%s_download_path"  %tv_movie))
        if _path:
...
def status( port,timeout=30):
        ...
        if not _shutdown:
            ...
            if 'error_code' in _last_status:
                logging.warning('(Torrent) %s: %s' %('Status', _last_status))
                sys.exc_clear()
...
def _debug( message):
        logging.warning("(Torrent) (torrent2http) %s" % message)
...
def preload(port):
    ...
    progress_tor='מפעיל'
    logging.warning('(Loader) Pre-Loading media')
    ...
    while not _shutdown:
        ...
        if not status['state'] == NO_CONNECTION:
            break
    #get file
    logging.warning('get file:')
    if not isAlive():
       raise TorrentError("torrent2http are not running")
    _last_files = get_html(_url+"/ls", timeout=5).json()['files']
    #_checkData
    logging.warning('_checkData:')
    while not _shutdown:
            status =  get_html(_url+ "/status", timeout=5).json() 
    ...
    for i, f in enumerate(files):
        ...
        logging.warning('(Torrent) File name: %s, MIME info: %s' %(f['name'], ''))
    ...
    while not _shutdown:
        ...
        if 'error' in data and len(data['error'])>1:
            logging.warning('Error in data:'+str(data['error']))
            err=data['error']
            stri2=err
        prog=0
        try:
            ...
            logging.warning('Buffer err:'+str(e))
            pass
        elapsed_time = time.time() - start_time
        if elapsed_time>timeout_magnet:
        ...
        if progress >= 100 or (filestatus['download']*0.45) >= needCacheSize: # We are caching about 65% (filestatus['download']*0.45) more end need (needCacheSize).
            ...
            logging.warning('(Loader) Finished with pre-loading media')
            if Addon.getSetting('new_play_window')=='false':
                dp.close()
            return f_url
...
def get_torrent_link(url_n,tv_movie):
    ...
    if not _shutdown:
        logging.warning('(Torrent) Find free port')
        port = get_free_port()

        logging.warning('(Torrent) Starting torrent2http')
        startupinfo = None
        if Platform.system == "windows":
        ...
        while not _shutdown:
            ...
            logging.warning('now_state:')
            logging.warning(now_state)
            if not now_state['state'] == NO_CONNECTION:
                logging.warning("(Torrent) torrent2http successfully started")
                break
    url=preload(port)
    return url