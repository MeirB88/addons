...
def GetJson(imdb,mode_subtitle,season=0,episode=0,version=0):
    ...
    if json_object!=0:
        for item_data in json_object:
            ...
            json_data={'url':url,
                             'label':"Hebrew",
                             'label2':item_data["versioname"]+' [B][COLOR dodgerblue][Wizdom][/COLOR][/B]',
    ...
    if item["tvshow"]:
          ...
          try:
            ...
            if mode_subtitle==3 and dp:
               dp.update(0, 'אנא המתן','33מחפש ידנ55', imdb_id )
            else:
...
def FirstPlace_Search(item,mode_subtitle,imdb_id):
    ...
    if 1:
        ...
        for itt in m_pre:
            ...
            json_data={'url':url,
                             'label':"Hebrew",
                             'label2':nm+' [B][COLOR khaki][Ktuvit][/COLOR][/B]',
                             'iconImage':"0",
                             'thumbnailImage':"he",
                             'hearing_imp':'false',
                             'sync': 'false'}
    ...
    while 1:
        for threads in thread:
              ...
              if all_setting["firstplace"]== 'true':
                 ...
                 string_dp=string_dp+('Screwzira:[COLOR %s]%s[/COLOR] '%(tt[zz],len( links_first)))
                 zz=zz+1
...
def search_all(mode_subtitle,all_setting,manual_search=False,manual_title=''):
    ...
    if mode_subtitle==3:
       ...
       if KODI_VERSION>18:
            ...
            dp.create('מקבל מידע...','מקבל מידע...','','')
    else:
       notify2('מקבל מידע...',all_setting)
    if mode_subtitle==1:
        try:
             ...
             if 1:#try:
                 if len(best_sub)>0:
                     ...
                     if 'language=Hebrew' not in best_sub and  'language=he' not in best_sub and all_setting["auto_translate"]=='true' and ('language=' in  best_sub or 'Hebrew' not in label):
                      notify2('[COLOR springgreen]מתרגם משפה זרה, אנא המתן...[/COLOR]',all_setting)
                      try:
